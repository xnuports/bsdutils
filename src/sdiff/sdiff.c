...
main(int argc, char **argv)
{
	...
	while ((ch = getopt_long(argc, argv, "aBbdEHI:ilo:stWw:",
		...
		case 't':
			...
			break;
		case 'w':
			wval = strtol(optarg, NULL, 10);
			if ((errno == EINVAL || errno == ERANGE) || (wval < WIDTH_MIN || wval > INT_MAX))
				errx(2, "width is %s", optarg);
			break;
		case HELP_OPT:
			...
			break;
		case TSIZE_OPT:
			tabsize = strtol(optarg, NULL, 10);
			if ((errno == EINVAL || errno == ERANGE) || (tabsize < 1 || tabsize > INT_MAX))
				errx(2, "tabsize is %s", optarg);
			break;
		default:
			usage();
			break;
...
parsecmd(FILE *diffpipe, FILE *file1, FILE *file2)
{
	...
	c = *p;
	*p++ = 0;
	file1start = strtol(line, NULL, 10);
	if ((errno == EINVAL || errno == ERANGE) || file1start > INT_MAX)
		errx(2, "file1 start is %s", line);
	...
	if (c == ',') {
		...
		c = *p;
		*p++ = 0;
		file1end = strtol(q, NULL, 10);
		if ((errno == EINVAL || errno == ERANGE) || file1end > INT_MAX)
			errx(2, "file1 end is %s", line);
	...
	c = *p;
	*p++ = 0;
	file2start = strtol(q, NULL, 10);
	if ((errno == EINVAL || errno == ERANGE) || file2start > INT_MAX)
		errx(2, "file2 start is %s", line);
	...
	if (c == ',') {
		...
		file2end = strtol(p, NULL, 10);
		if ((errno == EINVAL || errno == ERANGE) || file2end > INT_MAX)
			errx(2, "file2 end is %s", line);
